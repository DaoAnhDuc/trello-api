...
const getDetails = async (boardId) => {
  try {
    ...
    const result = await GET_DB().collection(BOARD_COLLECTION_NAME).aggregate(aggregate).toArray();
...
//Cap nhat gia tri column id vao cuoio mang ColumnOrderIds
...
const update = async (boardId, updateData) => {
  try {
    Object.keys(updateData).forEach((i) => {
      if (INVALID_UPDATE_FIELDS.includes(i)) {
        delete updateData[i];
      }
    });
    const result = await GET_DB()
